...
struct MANGOS_DLL_DECL instance_sunwell_plateau : public ScriptedInstance
{
    ...
    uint32 GetData(uint32 id)
    {
        switch(id)
        {
            ...
            case DATA_SATHROVARR_EVENT:   return Encounters[6]; break;
            case DATA_DECIVER:            return Encounters[7]; break;
            case DATA_ALYSTHESS_EVENT:    return Encounters[8]; break;
            case DATA_SACROLASH_EVENT:    return Encounters[9]; break;
        }
    ...
    void EjectPlayer(Player* plr)
    {
        ...
        // this part of code is refering to low level threat management what was disabled in MaNGOS 8867/8 
        // need to be redone
        // commented in oreder to restore compilation
        /*Creature* Sath = ((Creature*)Unit::GetUnit(*plr, Sathrovarr));
        if (Sath && Sath->isAlive())
        {
            HostileReference* ref = Sath->getThreatManager().getOnlineContainer().getReferenceByTarget(plr);
            if (ref)
            {
                ref->removeReference();
