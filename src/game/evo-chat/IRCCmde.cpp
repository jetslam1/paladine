...
void IRCCmd::Handle_Login(_CDATA *CD)
{
    ...
    if (isbanned == "NOTBANNED")
    {
            ...
            if (!AcctIsLoggedIn(_PARAMS[0].c_str()))
            {
                ...
                if (result)
                {
                    ...
                    if (GMLevel >= 0)
                    {
                        ...
                        Send_IRCA(CD->USER, MakeMsg("You Are Now Logged In As %s, Welcome To evo-Chat Admin Mode.", _PARAMS[0].c_str()), true, CD->TYPE);
        ...
        Send_IRCA(CD->USER, "\0034[ERROR] : Sorry You Are "+isbanned+". You Cannot Log In To evo-Chat "+CD->USER.c_str()+"!", true, "ERROR");
}
    ...
    if (result)
    {
        if (IsLoggedIn(CD->USER))
        {
            if (_PARAMS[0] == "")
            {
                ...
                if (result)
                {
                    std::string output = "\002evo-Chat IRC Commands:\017 ";
