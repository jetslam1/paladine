...
bool IRCClient::LoadConfig(char const* cfgfile)
{
    if (!MCConfig.SetSource(cfgfile))
        sLog.outString("*** evo-Chat: Unable to open configuration file (%s), All default options are being used.", cfgfile);
    else
        sLog.outString("*** evo-Chat: Found the configuration file, %s", cfgfile);
    ...
    sIRC._Nick = MCConfig.GetStringDefault("irc.nick", "evo-Chat");
    ...
    sIRC.JoinMsg = MCConfig.GetStringDefault("irc.joinmsg", "Whhaaazzzzaaaa, evo-Chat $Ver Baby!!");
    sIRC.RstMsg  = MCConfig.GetStringDefault("irc.rstmsg", "evo-Chat Is Restarting, I Will Be Right Back!");
